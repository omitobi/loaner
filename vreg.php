<script type="text/javascript">
	...
	if (isset($sub_reg)){
		...
		$regquery=mysql_query("INSERT INTO useres (uname,pword,fname,mname,lname,prev) VALUES ('$username','$ver_pass','$firstname','$middlename','$lastname','$aright')");
		if($regquery){
			$logreg=mysql_query("INSERT INTO reglog (registerer_id,reg_id,date_reg) VALUES ('".$_SESSION["user"]."','$username',now())");
			if($logreg){
				$_GET["action"]="nothing";
				$_GET["alert_msg"]="Form success!";
			} else{echo mysql_error()."YYYY";}
		} else {
			
			echo mysql_error()."YYYY";
			$_GET["alert_msg"]="Submission Error!";}
		
	} else{$_GET["alert_msg"]="Discrepancies in form".mysql_error();
	echo mysql_error()."YYYY";
	}
	}else {echo mysql_error()."YYYY";}
	?>
    ...
    <h2> Make Registration Entries:</h2>
    	<form action="" method="post" style=" position: static" name="regForm">
            ...
            <div class="dynamicbutton" style="margin-right:10%"><input type="submit" name="sub_reg" id="sub_reg" value="Register"/><input type="reset" /></div>
        </form>
    </div>