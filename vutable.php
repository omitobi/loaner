...
{
    $reslitab = "";
    $reslog = $conn->query("select * from useres")->fetchAll();       //userdata is the table name in tester1 database
    if (!empty($reslog)) {
        foreach ($reslog as $key => $reslii) {
        ...
        echo "<font color=\"#990000\">No Record in Table " . "'" . $tname . "' </font>";
    }
    return $reslitab;
}
